...
if (isset($_POST['login'])) {
    if ($_POST['username'] === $login && $_POST['password'] === $pass) {
        ...
        $_SESSION['username'] = $_POST['username'];  // Przechowanie nazwy użytkownika w sesji
    } else {
...
// Jeśli użytkownik jest zalogowany, wyświetlamy przycisk wylogowania
echo '<div style="text-align: right; padding: 10px;">
        Zalogowany jako ' . htmlspecialchars($_SESSION['username']) . ' | 
        <a href="?action=logout" style="text-decoration: none; color: black;">Wyloguj</a>
      </div>';
